...
public class UserControllerTest {
    ...
    @SneakyThrows
    @Test
    void shouldReturnStatus400IfBodyNotValid() {
        UserDto notValidDto = UserDto.builder().name("name").email("notValidMail.com").build();

        mvc.perform(post("/users")
                        .content(mapper.writeValueAsString(notValidDto))
                        .characterEncoding(StandardCharsets.UTF_8)
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON))
